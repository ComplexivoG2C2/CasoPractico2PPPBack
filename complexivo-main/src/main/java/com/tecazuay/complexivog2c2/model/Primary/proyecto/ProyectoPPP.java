...
public class ProyectoPPP implements Serializable {
    ...
    @OneToMany(targetEntity = ActividadesProyecto.class, mappedBy = "proyectoPPP")
    ...
    @OneToMany(targetEntity = ObjetivosEspecificosProyecto.class, mappedBy = "proyectoPPP")
    ...
    private void verifyEndDate() {
        if (this.fechaFin.after(new Date())) {
            this.estado = false;
            this.tutorAcademicoDelegados.forEach(d -> d.setEstado(false));
            this.tutorEmpresarial.setEstado(false);
        }
    }
}
